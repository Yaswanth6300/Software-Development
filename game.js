// game.js - Contains the complete game logic
...
function update() {
    ...
    jet.bullets.forEach(b => b.y -= 10 * jet.powerLevel);
    ...
    powerUps.forEach((p, pi) => {
        if (jet.x - jet.width / 2 < p.x + p.width &&
            ...
            jet.powerLevel = Math.min(jet.powerLevel + 1, 3);
...
function draw() {
    ...
    ctx.lineTo(jet.x - jet.width / 2, jet.y + jet.height / 2);
    ctx.lineTo(jet.x - jet.width / 3, jet.y);
    ctx.lineTo(jet.x + jet.width / 3, jet.y);
    ctx.lineTo(jet.x + jet.width / 2, jet.y + jet.height / 2);
    ctx.fill();
    ctx.fillStyle = '#00CED1';
    ctx.fillRect(jet.x - jet.width / 4, jet.y - 5, jet.width / 2, 5);

    // Draw bullets
    ctx.fillStyle = '#FF4444';
    jet.bullets.forEach(b => {
        ctx.fillRect(b.x, b.y - 5, 4, 10 * jet.powerLevel);
    });
